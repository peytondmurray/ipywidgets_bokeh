...
export class WidgetManager extends HTMLManager {
  ...
  async render(bundle: ModelBundle, el: HTMLElement, onDisplay: () => void): Promise<void> {
    ...
    try {
      ...
      if (model != null) {
        // const result: any = await this.display_model(undefined as any, model, {el})
  ...
  // async displayed(): Promise<void> {
  //   await Promise.all(Object.entries(this._known_models).map(([id, model]) => {
